...
export class Client {
	...
	async connect(): Promise<Connection> {
		...
		// MaxRequestId tells the relay how many outstanding requests it can send us.
		// Without this, the relay cannot forward SUBSCRIBE requests to the publisher (default 0 = no requests).
		const params = new Map<bigint, Uint8Array | bigint>()
		params.set(BigInt(SetupParameters.MaxRequestId), 100n)
